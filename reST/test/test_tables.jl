...
include2_path = joinpath(@__DIR__, "include2.txt")
include2_path_70len_rows =
    join(
        [
            @sprintf "|    %-70s    |" include2_path[ i : min(i+70-1,end) ]
            for i in 1:70:length(include2_path)
        ],
    )
include2_path_65len_rows =
    join(
        [
            @sprintf "              %-65s" include2_path[ i : min(i+65-1, end) ]
            for i in 1:65:length(include2_path)
        ],
    )
